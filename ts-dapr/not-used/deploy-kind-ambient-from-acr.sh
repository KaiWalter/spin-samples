...
do
  ...
  helm upgrade --install $app-dapr-ambient oci://registry-1.docker.io/daprio/dapr-ambient-chart \
    --set fullnameOverride=$app-dapr-ambient \
    --set ambient.appId=$app \
    --set ambient.remoteURL=$app \
